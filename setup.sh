...
function install() {
    install_arg=$1
    case ${install_arg} in
        -a)  install_docker-ce;install_nvidia-docker2;install_docker-compose     ;;
        *)   
        until  
        echo
        echo "1. 安装docker-ce"  
...
function remove() {
    remove_arg=$1
    case ${remove_arg} in
        -a)  remove_nvidia-docker2;remove_docker-ce;remove_docker-compose    ;;
        *)
        until  
        echo
...
function main() {
    Command=$1   
	    ...
	    case "${Command}" in
            ...
            check)      check   ;       exit 0  ;;
            *)  echo -e "需要执行命令，后面加上 --help 查看可执行命令的更多信息"    ;   exit 0  ;;
	    esac
}
